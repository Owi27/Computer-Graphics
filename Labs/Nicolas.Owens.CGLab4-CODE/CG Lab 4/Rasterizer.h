...
unsigned int BlendTriangle(unsigned int _color1, unsigned int _color2, unsigned int _color3, Vertex _vertex)
{
	...
	return blendedRed << 24 | blendedGreen << 16 | blendedBlue << 8;
}
...
void FillTriangle(Vertex _ver1, Vertex _ver2, Vertex _ver3)
{
	for (size_t j = 0; j < RasterHeight; j++)
	{
		for (size_t i = 0; i < RasterWidth; i++)
		{
			Vertex bya = BarycentricCoordinates(_ver1.X, _ver1.Y, _ver2.X, _ver2.Y, _ver3.X, _ver3.Y, i, j);
			if (bya.X >= 0 && bya.X <= 1 &&
				bya.Y >= 0 && bya.Y <= 1 &&
				bya.Z >= 0 && bya.Z <= 1)
			{
				FillPixel(PixelLocation(i, j), BlendTriangle(_ver1.ColorCode, _ver2.ColorCode, _ver3.ColorCode, bya));
			}
		}
	}
}
