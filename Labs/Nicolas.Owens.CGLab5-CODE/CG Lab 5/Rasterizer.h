...
void FillPixel(unsigned int _pixelLocation, unsigned int _colorCode, float _depth)
{
	if (_depth < ZBuffer[_pixelLocation])
	{
		Raster[_pixelLocation] = _colorCode;
		ZBuffer[_pixelLocation] = _depth;
	}
}
...
void Parametric(const Vertex& _vertex1, const Vertex& _vertex2, unsigned int _colorCode)
{
	...
	// CA: optimization - remove divide from loop
	float xDeltaReciprocal = 1.0f / xDelta;
	float yDeltaReciprocal = 1.0f / yDelta;
	...
	if (absX > absY)
	{
		...
		if (xDelta < 0)
		{
			for (size_t i = screenP1.X; i > screenP2.X; i--)
			{
				...
				float Ratio = (i - screenP1.X) * xDeltaReciprocal;
	...
	else
	{
		...
		if (yDelta < 0)
		{
			for (size_t i = screenP1.Y; i > screenP2.Y; i--)
			{
				float Ratio = (i - screenP1.Y) * yDeltaReciprocal;
...
float Burp(float _alpha, float _beta, float _gamma, Vertex _vertex)
{
	return _alpha * _vertex.X + _beta * _vertex.Y + _gamma * _vertex.Z;
}
...
unsigned int BlendTriangle(unsigned int _color1, unsigned int _color2, unsigned int _color3, Vertex _vertex)
{
	...
	unsigned int blendedAlpha = (Col1A * verAlpha) + (Col2A * verBeta) + (Col3A * verGamma);
...
void FillTriangle(Vertex _ver1, Vertex _ver2, Vertex _ver3)
{
	...
	// CA: do better brute here
	// calculate minY, maxY, minX, and maxX of tri, then change your loop below to go from min to max, instead of 0 to raster width/height
	int MinY = min(min(screenVer1.Y, screenVer2.Y), screenVer3.Y);
	int MaxY = max(max(screenVer1.Y, screenVer2.Y), screenVer3.Y);

	int MinX = min(min(screenVer1.X, screenVer2.X), screenVer3.X);
	int MaxX = max(max(screenVer1.X, screenVer2.X), screenVer3.X);

	for (size_t j = MinY; j < MaxY; j++) // plug minY, maxY in here
	{
		for (size_t i = MinX; i < MaxX; i++)
		{
			...
			if (bya.X >= 0 && bya.X <= 1 &&
				...
				bya.Z >= 0 && bya.Z <= 1)
			{
				float interZ = Burp(copyVer1.Z, copyVer2.Z, copyVer3.Z, bya);
				bya.U = Burp(_ver1.U, _ver2.U, _ver3.U, bya);
				bya.V = Burp(_ver1.V, _ver2.V, _ver3.V, bya);

				if (PixelShader != nullptr)
				{
					Pixel pixel;
					PixelShader(pixel, bya);
					FillPixel(PixelLocation(i, j), pixel.Color, interZ);
				}
				else
					FillPixel(PixelLocation(i, j), BlendTriangle(_ver1.ColorCode, _ver2.ColorCode, _ver3.ColorCode, bya), interZ);
			}
		}
	}
}

void BaryBlit(Vertex _ver1, Vertex _ver2, Vertex _3)
{
